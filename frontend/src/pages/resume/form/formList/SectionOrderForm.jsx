...
function SortableItem({ id }) {
  ...
  return (
    <li ref={setNodeRef} style={style} {...attributes} {...listeners} className="flex items-center justify-between p-3 bg-base-200 rounded-lg shadow mb-2 cursor-move">
...
export const SectionOrderForm = ({ resume, setResume, goToPrevStep, goToNextStep }) => {
  const apiUrl = import.meta.env.VITE_API_URL; // API URL from environment variables
  const [items, setItems] = useState(resume.section_order || Object.keys(defaultLabels)); // State for section order
  ...
  const saveSectionOrder = async () => {
    ...
    try {
      await axios.put(`${apiUrl}/cvs/${resume.id}/`, { ...resume, section_order: items });
  ...
  return (
    <div className="w-80 lg:w-[610px] md:w-[500px] mx-auto bg-base-100 shadow-md p-6 rounded-lg">
      ...
      <h2 className="text-xl font-bold text-info-content mb-6">Réorganiser les sections</h2>
      ...
      <div className="flex justify-between items-center mt-6">
        <button onClick={goToPrevStep} className="btn">Précédent</button>
